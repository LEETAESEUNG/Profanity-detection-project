...
#욕설 여부 검사
def is_swearing(content, swearing_words):
...
# 새로운 테이블 생성 및 저장
def create_and_populate_new_table(conn, cursor, contents, categories):
    cursor.execute("CREATE TABLE IF NOT EXISTS posts_category (content TEXT UNIQUE, category INTEGER)")
    for content, category in zip(contents, categories):
        cursor.execute(
            "INSERT INTO posts_category (content, category) VALUES (?, ?) "
            "ON CONFLICT(content) DO UPDATE SET category=excluded.category",
            (content, int(category))
        )
    conn.commit()
...
if __name__ == "__main__":
    ...
    # 욕설 여부 레이블 생성
    labels = [1 if is_swearing(text, swearing_words) else 0 for text in texts]
    ...
    # 새로운 테이블에 저장
    create_and_populate_new_table(conn, cursor, texts, all_predictions)
